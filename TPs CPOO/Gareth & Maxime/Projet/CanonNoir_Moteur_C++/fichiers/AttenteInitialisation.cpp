...
void AttenteInitialisation::execute(){
	if(((motor.getFacade())->_couleur1!=0 && (motor.getFacade())->_couleur2!=0) &&
		...
		(motor.getNbJoueurs()==4 && (motor.getFacade())->_couleur2!=0 && (motor.getFacade())->_couleur2!=0))) {
			motor.getJoueurInd(0) = (*new Joueur(1, motor.getPlateau()->getCasePort(motor.getFacade()->_couleur1)));
			motor.getJoueurInd(1) = (*new Joueur(2, motor.getPlateau()->getCasePort(motor.getFacade()->_couleur2)));
			motor.getJoueurInd(2) = (*new Joueur(3, motor.getPlateau()->getCasePort(motor.getFacade()->_couleur3)));
			motor.getJoueurInd(3) = (*new Joueur(4, motor.getPlateau()->getCasePort(motor.getFacade()->_couleur4)));
